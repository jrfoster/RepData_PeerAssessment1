...
Here we use `dplyr` to group the data by interval and calculate a mean number of steps.
...
and plot the data as a time series

Note in the plot the spike at an interval somewhere between 750 and 1000.  We calculate this exact interval by finding the maximum average

```{r}
max(avgSteps$avgSteps)
avgSteps[avgSteps$avgSteps == max(avgSteps$avgSteps, na.rm = TRUE), 1]
```

Based on this output, we can see that the maximum average 206.16981 occurs at interval 835.
